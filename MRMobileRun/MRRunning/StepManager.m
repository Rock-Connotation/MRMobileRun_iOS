...
@property(nonatomic,strong)NSMutableArray <StepModel *>*originalArray;
@property(nonatomic,strong)NSMutableArray <StepModel *>*stepArray;
...
+ (instancetype)defaultManager {
    ...
    dispatch_once(&onceToken, ^{
        if (manager == nil) {
            manager = [[StepManager alloc] init];
            manager.postUpdateTime = 1.0f; //发送更新通知的频率,停止通知不受其影响.默认为1秒;
            manager.minSpaceTime = 0.259;//两步之间允许的最小时间间隔 默认为259毫秒;
            manager.minStartTime = 2.0f;//计步器最小启动时间 默认为2秒;
            manager.maxStopTime = 10.0f;//计步器最长停止时间,大于该时间则停止 默认为10秒;
            manager.maxG = -0.12;//最大振动浮动,默认为 -0.12
        }
    });
    return manager;
}
...
- (void)start {
    ...
    //初始化数据
    manager.step = 0; //总计运动步数
    manager.second = 0; //总计运动用时
    manager.distance = 0; //估算距离
    manager.overTime = 0; //运动超时变量
    manager.postTime = 0; //发送时间变量
    manager.isEndSpeed = YES; //停止运动临时变量
    ...
    if (!manager.motionManager.isAccelerometerAvailable) {
        manager.motionState = MotionStateError; //错误状态,可能该设备不支持加速仪
        return;
    }
...
- (void)disposeAccelerometerDataWithG:(double)g {
    ...
    if (self.originalArray.count == 10) {
        ...
        for (int i = 1; i < bufferArray.count - 2; i++) {
            ...
            StepModel *bufferPrevious = (StepModel *)[bufferArray objectAtIndex:i-1];
            StepModel *bufferCurrent = (StepModel*)[bufferArray objectAtIndex:i];
            StepModel *bufferNext = (StepModel *)[bufferArray objectAtIndex:i+1];
...
// 根据采点数组进一步组合最后的数据
- (void)makeUpStepArrayActionWithCaiDianArray:(NSArray <StepModel *>*)caiDianArray {
    
    for (int i = 0; i < caiDianArray.count; i++) {

       StepModel *caidianCurrent = [caiDianArray objectAtIndex:i];
