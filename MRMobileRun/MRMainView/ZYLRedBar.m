...
#import "ZYLRedBar.h"
#import <Masonry.h>
@implementation ZYLRedBar
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.backgroundColor = COLOR_WITH_HEX(0xFF5C77);
        self.layer.cornerRadius = 6*kRateX;
...
//        barLayer.strokeColor = COLOR_WITH_HEX(0xFF5C77).CGColor;
...
- (void)setTextLabStr:(NSString *)textLabStr{
    ...
    self.textLab.frame = CGRectMake(5, 2.5, size.width, size.height);
...
//    CGSize size = [dataLabStr sizeWithAttributes:@{ NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Regular" size:14]}];
    [self.dataLab mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(size.width+10*kRateX);
...
//    textLayer.foregroundColor = COLOR_WITH_HEX(0xFFFFFF).CGColor;
...
////    tittleLayer.
//    tittleLayer.foregroundColor = COLOR_WITH_HEX(0xFFFFFF).CGColor;
