...
- (void)configurePropertys {
    ...
    _selectedTextFont = [UIFont boldSystemFontOfSize:15];
    _normalTextColor = COLOR_WITH_HEX(0xB2B2B2);
    if (@available(iOS 13.0, *)) {
        _selectedTextColor = [UIColor labelColor];
    } else {
        _selectedTextColor = COLOR_WITH_HEX(0x333739);
...
- (void)transitionFromCell:(UICollectionViewCell<TYTabPagerBarCellProtocol> *)fromCell toCell:(UICollectionViewCell<TYTabPagerBarCellProtocol> *)toCell animate:(BOOL)animate {
    ...
    void (^animateBlock)() = ^{
        if (fromCell) {
            fromCell.titleLabel.font = _selectedTextFont;
